from flask import Flask, request, render_template, jsonify , redirect, session   # Import jsonify

import ast  # If you're using ast.literal_eval
import numpy as np
import pandas as pd
import pickle
        ...
        if symptoms == "Symptoms":
            ...
            for symptom in patient_symptoms:
                ...
                if corrected_symptom:
                    ...
                    message = f"Symptom '{symptom}' not found, Renter the valid symptoms."
                    return render_template('index.html', message=message)

            predicted_disease = predicted_value(corrected_symptoms)
            session['predicted_disease'] = predicted_disease  # ✅ Store in session
            ...
            medications = [item for item in medication_list]

            diet_list = ast.literal_eval(rec_diet[0])
            rec_diet = [item for item in diet_list]
...
def home():
    if request.method == 'POST':
        symptoms = request.form.getlist('symptoms')  # get list from checkboxes
        if not symptoms:
            message = "Please select at least one symptom."
            return render_template('index.html', message=message)

        # Clean symptoms and correct spelling
        corrected_symptoms = []
        for symptom in symptoms:
            ...
            if corrected:
                ...
                message = f"Symptom '{symptom}' not recognized."
                return render_template('index.html', message=message)
        ...
        session['predicted_disease'] = predicted_disease  # Store for Google Maps search

        # Process lists
        medication_list = ast.literal_eval(medications[0])
        medications = [m for m in medication_list]
        diet_list = ast.literal_eval(rec_diet[0])
        rec_diet = [d for d in diet_list]
...
def search_maps():
    ...
    specialty = disease_specialty_map.get(disease, f"hospital for {disease}")

    # Build the Google Maps search URL
    query = f"{specialty} near {pincode}"
